...
def Map(InPdb, CGPrefix, AATraj = None, PrmTop = None, AmberEne = None, LastNFrames = 0, hasPseudoGLY = False):
    # read in protein structure
    p = protein.ProteinClass(Pdb = InPdb)
    p.Update()
    # hydrogenate if pseudo glycine sidechains are requested
    if hasPseudoGLY: p.Hydrogenate()
    PdbString = p.GetPdb()
    Pos = p.Pos
    ...
    SFilter_NoPseudoGLY = lambda ResName, AtomName : not (AtomName == 'N' or AtomName == 'CA' or AtomName == 'C' or AtomName == 'O' or AtomName.__contains__('H'))
    SFilter_PseudoGLY = lambda ResName, AtomName: (ResName == 'GLY' and ['HA2', '2HA'].__contains__(AtomName))
    ...
    for i, r in enumerate(Seq):
        ...
        SFilter = SFilter_PseudoGLY if r == 'GLY' and hasPseudoGLY else SFilter_NoPseudoGLY
